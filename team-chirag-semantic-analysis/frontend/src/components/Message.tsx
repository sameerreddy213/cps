      ...
      <div className={`flex max-w-[85%] ${isUser ? 'flex-row-reverse' : 'flex-row'} items-start space-x-3`}>
        ...
        <div className={`relative group ${isUser ? 'mr-3' : 'ml-3'}`}>
          <div className={`rounded-2xl px-4 py-3 ${
            ...
            <button
              onClick={handleCopy}
              className={`absolute top-2 ${isUser ? 'left-2' : 'right-2'} opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-black hover:bg-opacity-10 ${
                ...
                <div className="prose-invert">
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      pre: ({ children }) => (
                        <pre className="bg-black bg-opacity-20 rounded p-3 overflow-x-auto text-sm">
                          {children}
                        </pre>
                      ),
                    }}
            ...
            {message.links && message.links.length > 0 && (
              <div className="mt-3 space-y-2">
                {message.links.map((link, index) => (
                  <LinkPreview key={`${link.url}-${index}`} preview={link} />
                ))}
              </div>
            )}
          </div>
